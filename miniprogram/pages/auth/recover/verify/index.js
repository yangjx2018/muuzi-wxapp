...
Page({
  data: { email: '', code: '', left: RESEND, error: '', ready: false },
  ...
  onSubmit() {
    ...
    wx.navigateTo({ url: '/pages/auth/recover/password/index' });
  },
  ...
  async onResend() {
    ...
    try {
      ...
      await guduu.requestEmailCode(node, 'recovery', flow.email);
  ...
  goBack() {
    wx.navigateBack({ fail: () => wx.redirectTo({ url: '/pages/auth/recover/index' }) });
  },
});
